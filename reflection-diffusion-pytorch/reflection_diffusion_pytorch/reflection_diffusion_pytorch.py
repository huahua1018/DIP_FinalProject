...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        # when save .pth file, this will be saved as well
    ...
    def sample(self, batch_size = 16, img=None, t=None):
        ...
        while (t):
            ...
            x = img - xt_bar + xt_sub1_bar  # x_{t-1} = x_t - D(x_0,t) + D(x_0,t-1)
            img = x
            t = t - 1
    ...
    def train(self):
        ...
        while self.step < self.train_num_steps:
            ...
            for i in range(self.gradient_accumulate_every):
                ...
                #-------------------------------------------------
                # loss = torch.mean(self.model(data_1, data_2)) #TODO
                loss, mix = self.model(data_1, data_2)
            ...
            if self.step != 0 and self.step % self.save_and_sample_every == 0:
                ...
                #------------------------------------------
                #og_img = next(self.dl2).cuda()              # TODO: change og_img to dl1 or generate reflection from dl1
                og_img = data_1.cuda()
                xt = mix.cuda()
                #------------------------------------------

            
                # xt, direct_recons, all_images = self.ema_model.module.sample(batch_size=batches, img=og_img) # TODO

                xt, direct_recons, all_images = self.ema_model.module.sample(batch_size=batches, img=xt)

                og_img = (og_img + 1) * 0.5 # from [-1, 1] to [0, 1]
                ...
                utils.save_image(direct_recons, str(self.results_folder / f'sample-direct_recons-{milestone}.png'), nrow=6) # only one step from z to x
